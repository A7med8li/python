...
ptero = app_commands.Group(name="server", description="أوامر التحكم في سيرفر Pterodactyl")
...
@ptero.command(name="start", description="تشغيل السيرفر مع محاولة إضافته إلى طابور الانتظار")
async def start(interaction: discord.Interaction):
    global status_message, status_check_active
    
    if status_check_active:
        await interaction.response.send_message("يتم بالفعل متابعة حالة السيرفر", ephemeral=True)
        return
    ...
    if queue_message == "queue_unavailable":
        ...
        start_success, start_message = await send_ptero_power_command("start", "تشغيل")
        ...
        embed = discord.Embed(
            title="حالة السيرفر",
            description="⏳ تم إضافة السيرفر إلى طابور الانتظار، جاري التشغيل...",
            color=discord.Color.blue()
        )
        status_message = await interaction.followup.send(embed=embed)
